...
-- 테이블 레벨 제약조건 (모든열  선언 후 제약 조건을 취하는 방식)
CREATE TABLE dept2(
    dept_no NUMBER(2),
...
-- 외래 키(FOREIGN KEY)가 부모 테이블(참조테이블)에 없다면 INSERT가 불가능
INSERT INTO dept2
VALUES(10, 'gg', 6543, 9000, 'M'); -- 에러: 6543 지역은 없습니다.
...
-- 테이블 생성이후 제약조건 추가 및 변경, 삭제
-- 제약 조건은 추가, 삭제만 가능함. 변경은 안됨.
-- 변경하려면, 
CREATE TABLE dept2(
    dept_no NUMBER(2),
    dept_name VARCHAR2(14) NOT NULL,
...
-- 제약조건 삭제 (제약조건의 이름으로 -> 이름을 직접 짓지 않았다면 오라클이 부여한 이름을 제시)
