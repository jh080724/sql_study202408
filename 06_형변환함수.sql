...
-- 시간 포맷형식: HH[12] - 시(1~12), HH24 (0-23), MI - 분, SS - 초
...
SELECT 
    TO_CHAR(sysdate, 'YY"년" MM"월" DD"일" HH24:MI:SS') 
FROM dual; 
...
SELECT TO_DATE('2023-04-13 12:23:51', 'YY-MM-DD HH24:MI:SS') FROM dual;
...
FROM dual;
--------------------------------------------------------------
-- NULL 형태를 변환하는 함수 NVL(컬럼 or 값, 변환할 타겟값)
SELECT
    first_name,
    NVL(commission_pct, 0) AS COMM_PCT
FROM employees;

-- NULL 형태를 변환하는 함수 NVL2(target, null이아닐 경우, null일 경우)
SELECT
    first_name,
    salary,
...
-- commission_pct가 null인 애들은 뒤에 있는 0이 적용,
-- null이 아닌 애들은 자기의 commission_pct가 적용
SELECT
    first_name,
    salary,
